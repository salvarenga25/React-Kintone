...
app.get('/getData', cors(corsOptions), async (req, res) => {
  const fetchOptions ={
    method: 'GET',
    headers: {
      'X-Cybozu-API-Token': apiToken //depends on the db service you use, aws and other suppliers have different names
    }
  }
  //console.log(fetchOptions);
  const response = await fetch(multipleRecordsEndpoint, fetchOptions); //generate the response
  const jsonResponse = await response.json(); //you make the response into json
